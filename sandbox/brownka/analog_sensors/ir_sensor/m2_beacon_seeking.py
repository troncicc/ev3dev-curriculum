...
def main():
    ...
    try:
        while True:
            search = seek_beacon(robot)
            if search is True:
                print("Found the beacon!")
                ev3.Sound.speak("Found the beacon").wait()
            elif search is False:
                print("Ship lost at sea.")
                ev3.Sound.speak("Ship lost at sea.").wait()
        ...
        if current_distance == -128:
            ...
            if math.fabs(current_heading) < 2:
                ...
                if current_distance < 2:
                    ...
                    print("On the right heading. Distance: ", current_distance)
                    robot.button_forward(forward_speed, forward_speed)

            elif math.fabs(current_heading) < 10:
                if current_heading < 0:
                    print("Adjusting heading:", current_heading)
                    robot.button_left(turn_speed, turn_speed)
                else:
                    print("Adjusting heading:", current_heading)
                    robot.button_right(turn_speed, turn_speed)
